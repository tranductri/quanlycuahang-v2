...
-- Stock types per location (shared by opening and closing phases).
-- closing_field_key is the frontend payload field for the per-type closing count.
-- H1/H2 have no separate closing field yet (frontend sends only cuoi_thuc total).
-- Kho → cuoi_kho, Hộp → cuoi_hop are tracked individually.
INSERT INTO stock_types (location_id, name, field_key, closing_field_key, sort_order)
SELECT l.id, t.name, t.field_key, t.closing_field_key, t.sort_order
FROM locations l
CROSS JOIN (VALUES
  ('H1',  'dau_h1',  NULL,       0),
  ('H2',  'dau_h2',  NULL,       1),
  ('Kho', 'dau_kho', 'cuoi_kho', 2),
  ('Hộp', 'dau_cu',  'cuoi_hop', 3)
) AS t(name, field_key, closing_field_key, sort_order);
